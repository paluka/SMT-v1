...
public class PolyZone extends Zone{
	...
	public void drawZone(){
		...
		if (fill){
			TouchClient.parent.fill(colour.getRed(), colour.getGreen(), colour.getBlue(), colour.getAlpha());
		}
		TouchClient.parent.beginShape();
		if(this.curved){
			TouchClient.parent.curveVertex(coordinates[0], coordinates[1]);
		}
		
		for(int i = 0; i < coordinates.length; i = i + 2){

			if(this.curved){
				TouchClient.parent.curveVertex(coordinates[i], coordinates[i+1]);
			} else {
				TouchClient.parent.vertex(coordinates[i], coordinates[i+1]);
			}
		...
		TouchClient.parent.endShape(PConstants.CLOSE);
