...
public class TextZone extends RectZone{
	...
	String text;
	PFont font;
	PApplet applet = TouchClient.parent;
	int xAlign = -1;
	int yAlign = -1;
	float yOffset = 0;
	float xOffset = 0;
	...
	public static TextZone[] createWordZones(float x, float y, String text, PFont pfont, float textSize, float totalWidth, String regex, float yLineSpace, boolean upsideDown){
		...
		float leftSpacing = TouchClient.parent.getWidth()/120;
		float topSpacing = TouchClient.parent.getHeight()/90;
		float xx = 0;
		float yy = 0;
		...
		TouchClient.parent.textFont(pfont, textSize);
		TouchClient.parent.textAlign(PConstants.LEFT, PConstants.BOTTOM);
		float spaceWidth = TouchClient.parent.textWidth(' ');

		for(int j = 0; j < s.length; j++){
			char[] c = s[j].toCharArray();
			float textWidth = TouchClient.parent.textWidth(c, 0, c.length);
